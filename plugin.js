...
function settingsBrowser(resolveReturnUrl, mountCalibration, captureWeight, pitcherChoices, validateConfiguration, mountFlowPlan) {
  ...
  async function load() {
    try {
      ...
      gettingStartedText.append(make('strong', 'Getting started:'), make('span', ' Interpolate is the dividing line between simple exact calibrations and advanced flow modeling. Leave it off for the simplest setup: one calibration is enough. You can still save several exact calibrations at different flow and milk-target combinations and choose among them on the shot page. Turn Interpolate on only when you want continuously adjustable 0.1 ml/s flow at one milk target; that requires at least three readings—the minimum, maximum, and one in between—and unlocks Preview and Smooth curve fit.'));
      ...
      const instructions = [
        ...
        ['4 · Build a calibration library', 'With Interpolate off, each flow and milk-target combination is an independent exact calibration. One reading is enough, but you can save several and the shot page cycles through every calibration allowed by the Milk target filter. This is a useful middle ground between a single simple calibration and Interpolate.'],
        ['5 · How Interpolate chooses readings', 'Turn Interpolate on for the advanced path: it uses every saved reading at one selected milk target inside the selected range to make flow continuously adjustable. Other targets and out-of-range flows remain under Other saved calibrations.'],
      ...
      const attribution = make('p', 'Note: The calibrated steam-time formula and automatic pitcher-selection heuristic were inspired by Damian / Damian-AU’s DSx2. This is an independent JavaScript implementation for Decaid.');
      attribution.className = 'instructions-attribution'; panels.instructions.append(attribution);
